...
class SolidServerProvider implements IPAMProvider, DNSProvider {
    ...
    void addMissingIps(NetworkPool pool, List addList) {
        ...
        List<NetworkPoolIp> poolIpsToAdd = addList?.collect { it ->
            ...
            def addConfig = [networkPool: pool, networkPoolRange: pool.ipRanges ? pool.ipRanges.first() : null, ipType: ipType, hostname: name, ipAddress: ipAddress, externalId:it.ip_id]
    ...
    void updateMatchedIps(List<SyncTask.UpdateItem<NetworkPoolIp,Map>> updateList) {
        ...
        updateList?.each {  update ->
            ...
            if(existingItem) {
                ...
                def ipType = 'assigned'
                if(update.masterItem.type?.contains('free')) {
                    ipType = 'unmanaged'
                }
                def save = false
